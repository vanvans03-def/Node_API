...
const ProductPrice =  mongoose.model("ProductPrice", ProductPrice);
...
for(let i = 0 ;i < fruits.length;i++){
    axios.schedule('0 0 * * *', async () => {
        try{
            for(let i = 0; i <fruits.length; i++){
            const resp =  await axios.get(
                'https://dataapi.moc.go.th/gis-product-prices?product_id=${fruits[i]}&from_date=${date}&to_date=${yesterday}');
                const { product_id, product_name, category_name, group_name, unit, price_list} = resp.data;
            ...
            console.log('Date saved True');
                ...
                console.error('Error saved faile TT');
        }
    });
}